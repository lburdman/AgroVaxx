 ...
 * W AND R BY COOQROBOT. _Please_ see the comments in MFRC522.h - they give
 * useful hints and background. Released into the public domain.
 */
...
#define delay(ms) ThisThread::sleep_for(std::chrono::milliseconds(ms))

static const char *const _TypeNamePICC[] = {
    "Unknown type", "PICC compliant with ISO/IEC 14443-4",
    "PICC compliant with ISO/IEC 18092 (NFC)", "MIFARE Mini, 320 bytes",
    "MIFARE 1KB", "MIFARE 4KB", "MIFARE Ultralight or Ultralight C",
    "MIFARE Plus", "MIFARE TNP3XXX",
...
MFRC522::MFRC522(PinName mosi, PinName miso, PinName sclk, PinName cs,
                 PinName reset)
    : m_SPI(mosi, miso, sclk), m_CS(cs), m_RESET(reset) {
  /* Configure SPI bus */
  m_SPI.format(8, 0);
...
void MFRC522::PCD_ReadRegister(uint8_t reg, uint8_t count, uint8_t *values,
  ...
  values[index] =
      m_SPI.write(0); // Read the final byte. Send 0 to stop reading.
 ...
 */
uint8_t MFRC522::PCD_CalculateCRC(uint8_t *data, uint8_t length,
                                  uint8_t *result) {
  PCD_WriteRegister(CommandReg, PCD_Idle); // Stop any active command.
  PCD_WriteRegister(DivIrqReg, 0x04); // Clear the CRCIRq interrupt request bit
  PCD_SetRegisterBits(FIFOLevelReg,
  ...
  while (1) {
    ...
    if (--i == 0) {
      ...
      // 89ms. Communication with the MFRC522 might be down.
...
void MFRC522::PCD_Init() {
  ...
  // start up time of the crystal + 37,74us. Let us be generous: 50ms.
  delay(50);
  ...
  PCD_WriteRegister(
      ...
      0xA9); // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 =>
             // f_timer=40kHz, ie a timer period of 25us.
  PCD_WriteRegister(
      TReloadRegH,
...
void MFRC522::PCD_Reset() {
  ...
  // oscillator start-up time is the start up time of the crystal + 37,74us. Let
  // us be generous: 50ms.
  delay(50);
 ...
 */
uint8_t MFRC522::PCD_TransceiveData(uint8_t *sendData, uint8_t sendLen,
...
/**
 * Transfers data to the MFRC522 FIFO, executes a commend, waits for completion
 * and transfers data back from the FIFO. CRC validation can only be done if
 * backData and backLen are specified.
 */
uint8_t MFRC522::PCD_CommunicateWithPICC(uint8_t command, uint8_t waitIRq,
  ...
  if (command == PCD_Transceive) {
    PCD_SetRegisterBits(BitFramingReg,
 ...
 * STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t MFRC522::PICC_REQA_or_WUPA(uint8_t command, uint8_t *bufferATQA,
                                   uint8_t *bufferSize) {
  uint8_t validBits;
  uint8_t status;
  ...
  // - transmit only 7 bits of the last (and only) byte. TxLastBits =
  // BitFramingReg[2..0]
  validBits = 7;
...
uint8_t MFRC522::PICC_Select(Uid *uid, uint8_t validBits) {
  ...
  uint8_t currentLevelKnownBits; // The number of known UID bits in the current
                                 // Cascade Level.
  ...
  //    PICC_CMD_SEL_CL2 or PICC_CMD_SEL_CL3 Byte 1: NVB         Number of Valid
  //    Bits (in complete command, not just the UID): High nibble: complete
  //    bytes, Low nibble: Extra bits. Byte 2: UID-data or CT    See explanation
  //    below. CT means Cascade Tag. Byte 3: UID-data Byte 4: UID-data Byte 5:
  //    UID-data Byte 6: BCC         Block Check Character - XOR of bytes 2-5
  //    Byte 7: CRC_A
  //    Byte 8: CRC_A
  ...
  while (!uidComplete) {
    ...
    if (bytesToCopy) {
      // Max 4 bytes in each Cascade Level. Only 3 left if we use the Cascade
      // Tag
    ...
    while (!selectDone) {
      ...
      if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level
        ...
        // Serial.print("SELECT: currentLevelKnownBits=");
  ...
  if (result == STATUS_OK) {
    ...
    if (result == STATUS_TIMEOUT) {
      result = STATUS_OK;
               ...
               STATUS_OK) { // That is ironically NOT ok in this case ;-)
      result = STATUS_ERROR;
    }
  }
 ...
 */
uint8_t MFRC522::PCD_Authenticate(uint8_t command, uint8_t blockAddr,
 ...
 */
uint8_t MFRC522::MIFARE_Read(uint8_t blockAddr, uint8_t *buffer,
                             uint8_t *bufferSize) {
 ...
 */
uint8_t MFRC522::MIFARE_Write(uint8_t blockAddr, uint8_t *buffer,
                              uint8_t bufferSize) {
  uint8_t result;
 ...
 */
uint8_t MFRC522::MIFARE_UltralightWrite(uint8_t page, uint8_t *buffer,
                                        uint8_t bufferSize) {
  uint8_t result;
 ...
 */
uint8_t MFRC522::MIFARE_TwoStepHelper(uint8_t command, uint8_t blockAddr,
                                      uint32_t data) {
  uint8_t result;
 ...
 * MF_ACK or a timeout.
 */
uint8_t MFRC522::PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen,
                                       bool acceptTimeout) {
  uint8_t result;
 ...
 */
void MFRC522::MIFARE_SetAccessBits(uint8_t *accessBitBuffer, uint8_t g0,
